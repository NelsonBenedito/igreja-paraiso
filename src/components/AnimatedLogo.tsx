...
const AnimatedLogo: React.FC<AnimatedLogoProps> = ({ className = "", size = 600 }) => {
  ...
  const oliveiraGroupVariants: Variants = {
    ...
    visible: {
      opacity: 1,
      transition: { staggerChildren: 0.6, delayChildren: 1.2 },
    },
  };
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center w-full px-4 ${className}`}>
      {/* Container para centralizar a logo asimétrica */}
      <div className="w-full flex justify-center">
        <motion.svg
          style={{ width: '100%', maxWidth: size, height: 'auto' }}
          viewBox="-10 -10 435.13 135" 
          fill="none"
        ...
        <motion.g variants={oliveiraGroupVariants}>
          ...
          {/* Folha Esquerda */}
          <motion.path
            d="M50.04,74.02c3.6-13.69,4.23-26.91-3.42-39.54-.12-.24-.78-.53-1-.42-.17.08-.6.44-.68.65-5.83,13.73-6.77,28.9,3.37,41.04.04.05.07.12.11.17.56-.32.94-1.51,1.62-1.9Z"
            fill={colorWhite}
            variants={{
              hidden: { scale: 0.3, opacity: 0, rotate: -10, originX: 0.5, originY: 0.8 },
              visible: { scale: 1, opacity: 1, rotate: 0 },
            }}
            transition={{ duration: 2.4, ease: [0.34, 1.56, 0.64, 1] }}
          />

          {/* Folha Direita */}
          <motion.path
            d="M80.14,63.94c-.31-.11-1.06-.13-1.4.02-10.88,4.43-23.06,10.89-25.5,23.57-.08.43-.05.83-.06,1.16.1.33.26.66.52,1,8.38-2.82,15.82-7.89,21.07-15.05.87-1.19,7.51-9.96,5.37-10.7Z"
            fill={colorWhite}
            variants={{
              hidden: { scale: 0.3, opacity: 0, rotate: 10, originX: 0.5, originY: 0.8 },
              visible: { scale: 1, opacity: 1, rotate: 0 },
          ...
          {/* Folha Topo */}
          <motion.path
            d="M64.26,52.52c-.51-3.55-.19-7.01.93-10.42.95-2.92,1.14-5.69,1.4-8.71l.93,2.72,3.28-6.17c.46-.86.88-1.69,1.76-2.24l-4.17,9.6c-1.58,3.78-4.67,10.31-3.24,14.24,10.15-5.33,15.88-16.5,14.82-27.74-.17-1.78-.98-8.4-2.94-8.59-1.63-.15-6.09,4.24-7.59,5.69-8.01,7.78-12.04,20.32-6.83,30.76,0,0,0,0,0,0,.46.41.99.72,1.59.93l.05-.09Z"
            fill={colorWhite}
            variants={{
              hidden: { scale: 0.4, opacity: 0, rotate: -5, originX: 0.5, originY: 0.7 },
              visible: { scale: 1, opacity: 1, rotate: 0 },
            }}
            transition={{ duration: 2.1, ease: [0.34, 1.56, 0.64, 1] }}
          />

          {/* Fruto */}
          <motion.path
            d="M60.81,64.22c3.71,3.7,15.09-2.94,13.1-7.14-1.41-2.97-10.16-1.08-13.14,4.4-.18.34-.31.69-.46,1.01.22.51.23,1.14.36,1.76.05-.01.09-.03.14-.04Z"
            fill={colorWhite}
            variants={{
              hidden: { scale: 0.2, opacity: 0 },
              visible: { scale: 1, opacity: 1 },
            }}
            transition={{ duration: 1.8, ease: "easeOut", delay: 2.8 }}
          />
        </motion.g>

        {/* Texto "Igreja" e "Paraíso" - branco */}
      ...
      {/* Referência em Estilo Badge - Perfeitamente Simétrica e Responsiva */}
      <motion.div 
