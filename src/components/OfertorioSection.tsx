...
import type { SiteGiving } from '@/lib/site-content/types';

interface OfertorioSectionProps {
    content: SiteGiving;
}

const OfertorioSection: React.FC<OfertorioSectionProps> = ({ content }) => {
    const [copied, setCopied] = useState(false);

    // Conta/CNPJ vazios = não mostrar o card (placeholders não devem parecer reais).
    const showBankTransfer = Boolean(content.account.trim() && content.holderDocument.trim());
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10">
                ...
                <div className={`grid grid-cols-1 ${showBankTransfer ? 'md:grid-cols-2' : 'md:grid-cols-1 max-w-xl mx-auto'} gap-8 mx-auto mb-14`}>
                    {/* Card PIX — TODO: confirmar chave com a tesouraria (domínio antigo) */}
                    {content.pixKey.trim() ? (
                        <Reveal>
                                ...
                                <div className="space-y-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-2xl p-5 mt-auto text-sm text-slate-700 dark:text-slate-300 font-medium">
                                    ...
                                    <div className="flex justify-between pb-1">
                                        ...
                                        <span className="font-black text-slate-800 dark:text-white text-right">
                                            {content.holderName}<br />
                                            <span className="text-[10px] text-slate-400 font-normal">CNPJ: {content.holderDocument}</span>
                                        </span>
                                    </div>
                                </div>
                            </div>
                ...
                {/* Campanha de cotas — fora do contrato site-content; mantida local */}
